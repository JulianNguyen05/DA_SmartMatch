/**
 * cvTemplateCore.js
 * Các utility dùng chung cho tất cả CV templates (Simple, Harvard, Professional).
 * Import những gì bạn cần — không import toàn bộ file.
 */

// ─────────────────────────────────────────────
// 1. CSS CLASS CONSTANTS
// ─────────────────────────────────────────────

/** Class chung cho contentEditable: focus ring, placeholder, empty border đỏ */
...
/** Class riêng cho contact item (không có empty:border đỏ) */
...
/**
 * Lấy innerHTML đã trim, thay <br> rỗng thành ''.
 * Dùng trong onBlur của contentEditable.
 */
...
/**
 * Highlight wrapper class — thêm outline vàng khi section đang được chọn trong sidebar.
 */
export const highlightClass = (isHighlighted) =>
  isHighlighted
    ? 'outline outline-2 outline-dashed outline-yellow-400 p-2 rounded'
    : '';

/**
 * Map tỉ lệ layout row → Tailwind col-span classes (grid-cols-10).
 */
...
/**
 * Chuyển data từ cvData sang định dạng flat { date, title, subtitle, description }
 * để EditableTimelineList / EditableRowList / EditableTagList / EditableParagraphList hiểu được.
 */
export const adaptDataForList = (data, type) => {
  ...
  return data.map((item) => {
    switch (type) {
      case 'experience':
        return { date: item.duration, title: item.company, subtitle: item.role, description: item.description };
      case 'education':
        return {
          date: item.duration,
          title: item.school,
          subtitle: item.major,
          description: item.gpa ? `GPA: ${item.gpa}\n${item.description || ''}` : item.description,
        };
      case 'activities':
        return { date: item.duration, title: item.organization, subtitle: item.role, description: item.description };
      case 'projects':
        return { date: item.duration, title: item.name, subtitle: item.role, description: item.description };
      case 'certifications':
        return { date: item.date, title: item.name, subtitle: item.issuer, description: '' };
      case 'awards':
        return { date: item.date, title: item.title, subtitle: item.issuer, description: '' };
      case 'references':
        return {
          date: '',
          title: item.name,
          subtitle: item.position ? `${item.position} - ${item.company}` : item.company,
          description: '',
        };
      case 'skills':
        return { date: '', title: item.name, subtitle: '', description: item.description };
      case 'hobbies':
        return { date: '', title: item.name, subtitle: '', description: '' };
      default:
        return item;
    }
  });
...
/**
 * Chuyển ngược từ flat list về định dạng cvData gốc.
 */
export const revertDataFromList = (listData, type) => {
  ...
  return listData.map((item) => {
    switch (type) {
      case 'experience':
        return { duration: item.date, company: item.title, role: item.subtitle, description: item.description };
      case 'education': {
      ...
      case 'activities':
        return { duration: item.date, organization: item.title, role: item.subtitle, description: item.description };
      case 'projects':
        return { duration: item.date, name: item.title, role: item.subtitle, description: item.description };
      case 'certifications':
        return { date: item.date, name: item.title, issuer: item.subtitle };
      case 'awards':
        return { date: item.date, title: item.title, issuer: item.subtitle };
      case 'references': {
...
/**
 * Gắn MutationObserver lên containerRef để tự ngắt trang khi section tràn.
 * Dùng trong useEffect của mỗi template.
 *
 * @param {React.RefObject} containerRef
 * @param {any[]} deps - các giá trị trigger re-paginate (thường là [layout, data])
