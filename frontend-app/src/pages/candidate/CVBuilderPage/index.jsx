...
const CVBuilderPage = () => {
  ...
  // === THÊM KÍNH LÚP (RESIZE OBSERVER) ĐỂ TÍNH TOÁN SỐ TRANG ===
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (let entry of entries) {
        ...
        // Trừ hao 5px để tránh việc sai số thập phân làm nhảy trang ảo
      ...
      activeRows: [
        {
          id: "row-1",
          ratio: "10-0",
          leftItems: ["avatar", "personalInfo", "contactInfo", "objective"],
          rightItems: [],
        },
        {
          id: "row-2",
          ratio: "50-50",
          leftItems: ["experience", "education"],
          rightItems: ["skills", "hobbies"],
        },
      ],
      unusedItems: [
        "activities",
  ...
  const handleDragEnd = (event) => {
    ...
    newLayout.unusedItems.sort((a, b) => {
      ...
      // Nếu không có trong danh sách chuẩn (vd: lỗi), đẩy nó xuống chót (99)
  ...
  const handleFontChange = (font) =>
    setCvData((prev) => ({ ...prev, settings: { ...prev.settings, font } }));
    ...
    <div className="flex flex-col h-screen bg-[#f3f4f6]">
      <header className="h-14 bg-white border-b flex items-center justify-between p-4 shadow-sm z-20 font-bold text-gray-700">
        ...
        <button
          ...
          className="bg-[#00b14f] text-white px-4 py-1.5 rounded text-sm hover:bg-green-600"
        >
          Lưu CV
        </button>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-300 relative flex justify-center py-10 transition-all" style={{ marginLeft: isPanelOpen ? "10px" : "0" }}>
          ...
          >
            {/* Tự động tính toán để chỉ render các nhãn trang cần thiết */}
            {totalPages > 1 && Array.from({ length: totalPages - 1 }).map((_, i) => {
              const pageIndex = i + 1;
              return (
                <div 
                  key={pageIndex} 
                  className="absolute left-[-55px] bg-[#00b14f] text-white text-[10px] px-2 py-1 rounded shadow-sm z-50 font-medium"
                  style={{ top: `${pageIndex * 1123}px`, transform: 'translateY(-50%)' }}
                >
                  Trang {pageIndex + 1}
                </div>
              );
            ...
            <SelectedTemplate 
              cvData={cvData} 
              onSectionClick={(id) => console.log("Edit", id)}
